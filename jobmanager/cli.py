...
_LOG_DIR = "job_logs"
_CONFIG_PATH = os.path.join(os.path.expanduser("~"), ".config", "jobmanager", "config.yaml")


def _fetch_metadata(jobid: str, output_file: str, retries: int = None, retry_wait: int = 5) -> None:
    """Fetch sacct JSON metadata for *jobid* into *output_file*, retrying until the record appears."""
    if retries is None:
        retries = _load_config()["sacct_retries"]
...
class Job(JobMetadata):
    ...
    def update(self, retries: int = None, retry_wait: int = 5):
...
def _load_config() -> dict:
    """Load ~/.config/jobmanager/config.yaml and merge with built-in defaults."""
    import yaml
    config = {
        "tmp_dir_var": None,
        "module_loads": [],
...
def _config_init() -> None:
    """Create a default configuration file at _CONFIG_PATH."""
    if os.path.exists(_CONFIG_PATH):
        raise SystemExit(f"Config file already exists: {_CONFIG_PATH}")
    os.makedirs(os.path.dirname(_CONFIG_PATH), exist_ok=True)
    content = (
        "# jobmanager configuration file\n"
        "# Generated by: jobmanager config init\n"
        "\n"
        "# Variable name holding the compute node's local temporary directory.\n"
        "# Examples: TMPDIR, LOCAL_SCRATCH, SCRATCH\n"
        "# If unset, virtualenvs are created in the job's working directory (.venv).\n"
        "# tmp_dir_var: TMPDIR\n"
        "\n"
        "# Environment modules to load before creating a virtualenv (requirements.txt).\n"
        "# module_loads:\n"
        "#   - python/3.11\n"
        "#   - cuda/12.0\n"
        "\n"
        "# Directory for SLURM job log files (relative to the working directory).\n"
        f"log_dir: {_LOG_DIR}\n"
        "\n"
        "# Number of times to retry sacct when a job is not yet registered.\n"
        "sacct_retries: 6\n"
    )
...
def _config_edit() -> None:
    """Open the config file in an interactive editor."""
    if not os.path.exists(_CONFIG_PATH):
        print(f"Config file not found. Creating it first...")
        _config_init()
    editor = (
    ...
    if not editor:
        raise SystemExit(
            ...
            "or install nano or vi."
        )
    subprocess.run([editor, _CONFIG_PATH])


def _find_editor() -> str:
    """Return the path of the first available fallback editor."""
    import shutil
...
def _create_template(quick: bool = False, medium: bool = False, test: bool = False) -> None:
    ...
    # Build optional venv setup block when requirements.txt is present
    venv_block = ""
    if os.path.exists("requirements.txt"):
        module_loads = cfg.get("module_loads") or []
        tmp_dir_var = cfg.get("tmp_dir_var")
        if module_loads:
            venv_block += "".join(f"module load {m}\n" for m in module_loads)
        if tmp_dir_var:
            venv_dir = f"${{{tmp_dir_var}}}/.venv_${{SLURM_JOBID}}"
        else:
            venv_dir = ".venv"
        venv_block += (
    ...
    content = (
        ...
        f"#SBATCH --output={log_dir}/LOG_%x_%j.log\n"
        "\n"
        "sleep 10\n"
    ...
    if venv_block:
        content += venv_block + "\n"
    content += "# === INSERT YOUR CODE BELOW ===\n"
    ...
    if venv_block:
        if cfg.get("tmp_dir_var"):
            print(f"  → requirements.txt detected: venv setup added (using ${cfg['tmp_dir_var']}).")
        else:
            print("  → requirements.txt detected: venv setup added (using .venv in working directory).")
            print("     Tip: set 'tmp_dir_var' in your config for faster node-local storage.")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # config
    p_config = subparsers.add_parser("config", help="Manage jobmanager configuration.")
    config_sub = p_config.add_subparsers(dest="config_command", metavar="SUBCOMMAND")
    config_sub.required = True
    config_sub.add_parser("init", help=f"Create a config file at {_CONFIG_PATH}.")
    config_sub.add_parser("edit", help="Open the config file in an interactive editor.")
...
def main():
    ...
    if args.command == "status":
        ...
        if not _check_and_handle_active_job():
            return
        os.makedirs(_load_config()["log_dir"], exist_ok=True)
        try:
            result = subprocess.run(
